...
@Service
public class PikoServiceImpl implements PicoService {
	...
	@HystrixCommand(fallbackMethod = "getAppleDataFallback")
	public String getAppleData() {
		return getRestData(appleApi, "api/v1/apple");
	}
	
	@HystrixCommand(fallbackMethod = "getPenDataFallback")
	...
	private String getRestData(URI uri, String segument) {
		return restTemplate.getForObject(UriComponentsBuilder
				.fromUri(uri).pathSegment(segument).build().toUri(),
				String.class);
	}

	@SuppressWarnings("unused")
	private String getAppleDataFallback() {
		return "パイナップル";
	}
	
	@SuppressWarnings("unused")
	private String getPenDataFallback() {
		return "ペン";
	}
}
